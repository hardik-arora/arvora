...
Generated by Arvora (India City Autocomplete & Planner) 🚀`;
  ...
  function initNavigationHub() {
    const trigger = document.getElementById("omni-spotlight-trigger");
    const modal = document.getElementById("omni-command-modal");
    const searchInput = document.getElementById("omni-search-input");
    const pills = modal ? modal.querySelectorAll(".cat-pill") : null;
    const dock = document.getElementById("floating-mac-dock");
    ...
    if (dockTrigger) dockTrigger.addEventListener("click", openModal);
    ...
    document.addEventListener("keydown", e => {
      ...
      if (modal && modal.style.display === "flex") {
        const visibleItems = Array.from(modal.querySelectorAll(".omni-command-item")).filter(item => item.style.display !== "none");
        let activeIdx = visibleItems.findIndex(item => item.classList.contains("active"));

        if (e.key === "ArrowDown") {
          e.preventDefault();
          if (activeIdx !== -1) {
            visibleItems[activeIdx].classList.remove("active");
          ...
          if (activeIdx !== -1) {
            ...
            activeIdx = visibleItems.length - 1;
          }
          if (visibleItems[activeIdx]) {
            visibleItems[activeIdx].classList.add("active");
            visibleItems[activeIdx].scrollIntoView({ block: "nearest" });
          ...
          e.preventDefault();
          if (activeIdx !== -1 && visibleItems[activeIdx]) {
    ...
    const commandItems = modal ? modal.querySelectorAll(".omni-command-item") : [];
    commandItems.forEach(item => {
      item.addEventListener("click", () => {
        const tabId = item.id.replace("tab-btn-", "");
        switchTab(tabId);
        closeModal();
      });
    });
    ...
    function filterCommands() {
      ...
      if (modal) {
        const groupLabels = modal.querySelectorAll(".omni-group-label");
        groupLabels.forEach(label => {
          const groupName = label.dataset.group;
          const visibleInGroup = Array.from(commandItems).some(item => {
            return item.dataset.cat === groupName && item.style.display !== "none";
          });
          label.style.display = visibleInGroup ? "block" : "none";
        });
      }
    }
