...
Generated by Arvora (India City Autocomplete & Planner) 🚀`;
  ...
  function initCabEstimator() {
    ...
    const DATA = [
      { title: "Uber & Ola Airport Rides (Delhi / Mumbai / BLR)", icon: "🚗", tag: "App Booking", price: "₹18 - ₹24 / km", desc: "Dedicated pick-up zones at Airport Terminal Arrivals (IGI T3, BOM T2, KIA BLR). Airport toll (₹100-150) included in final fare." },
      { title: "BluSmart All-Electric Cabs (Delhi NCR / BLR)", icon: "⚡", tag: "Zero Surge", price: "₹15 - ₹20 / km", desc: "100% Electric EV cabs with zero surge pricing, clean seats & 99% on-time pickup guarantee. Advance airport scheduling available." },
      { title: "Rapido Bike Taxi & Auto Rides", icon: "🛵", tag: "Solo & Fast", price: "₹8 - ₹12 / km", desc: "Fastest way to cut through peak traffic jams in BLR, Hyderabad, Chennai & Delhi. Helmet provided by captain." }
    ];
    grid.innerHTML = DATA.map(item => `
      <div class="trip-stop-card" style="padding:1.25rem; background:rgba(15,23,42,0.6); border:1px solid var(--card-border); border-radius:16px; display:flex; flex-direction:column; justify-content:space-between;">
        <div>
  ...
  function initSignalSimplified() {
    ...
    if (!grid || grid.children.length > 0) return;
    const DATA = [
      { title: "Jio True 5G Coverage (Pan-India 700MHz + 3500MHz)", icon: "📡", tag: "Fastest 5G", price: "Free Unlimited 5G on ₹239+ Packs", desc: "Standalone 5G network (SA) providing 300Mbps+ speeds across all 28 states & metro underground lines." },
      { title: "Airtel 5G Plus (NSA 5G + Wide Rural Coverage)", icon: "📶", tag: "Best Remote Signal", price: "Unlimited 5G on ₹299+ Packs", desc: "Non-standalone 5G with superior indoor penetration & high-altitude coverage in Ladakh, Himachal & North East." },
      { title: "Vi (Vodafone Idea) 4G Unlimited Night Data", icon: "🌙", tag: "Night Binge", price: "Unlimited 4G (12 AM - 6 AM)", desc: "Free unthrottled 4G data between midnight & 6 AM + weekend data rollover." }
    ];
    grid.innerHTML = DATA.map(item => `
      <div class="trip-stop-card" style="padding:1.25rem; background:rgba(15,23,42,0.6); border:1px solid var(--card-border); border-radius:16px; display:flex; flex-direction:column; justify-content:space-between;">
        <div>
  ...
  function initAirportSecurity() {
    ...
    if (!grid || grid.children.length > 0) return;
    const DATA = [
      { title: "DigiYatra Facial Biometric Fast-Track Entry", icon: "🛂", tag: "Zero Wait Time", price: "Free Govt Service", desc: "Skip security queues at DEL, BOM, BLR, HYD, CCU, MAA, PNQ airports using facial recognition app." },
      { title: "Cabin Baggage Liquid & Powerbank Rules", icon: "💼", tag: "CISF Security Mandate", price: "Strict 100ml / 20,000mAh limit", desc: "All powerbanks must be in hand baggage only (max 100Wh). Liquids over 100ml prohibited in cabin baggage." },
      { title: "Laptops & Electronics Tray Placement", icon: "💻", tag: "X-Ray Screening", price: "Remove from Sleeve", desc: "Laptops, tablets, camera bodies, chargers & power cables must be placed in separate security trays." }
    ];
    grid.innerHTML = DATA.map(item => `
      <div class="trip-stop-card" style="padding:1.25rem; background:rgba(15,23,42,0.6); border:1px solid var(--card-border); border-radius:16px; display:flex; flex-direction:column; justify-content:space-between;">
        <div>
